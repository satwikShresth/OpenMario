...
import { FilterSchema, handleShare, type Filter } from '#/utils';
...
export const Route = createFileRoute('/home')({
  ...
  beforeLoad: ({ search, cause }) => {
    const { setSearch, company, position, location, year, coop_year, coop_cycle, program_level, distinct, pageIndex, pageSize
  ...
  component: () => {
    ...
    useEffect(() => {
      const unsubscribePositionStore = useFilterStore
        .subscribe(
          ({ company, position, location, year, coop_year, coop_cycle, program_level, distinct, pageIndex, pageSize, }) => {
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'flex-end', mb: 2 }}>
                <Button
                  ...
                  onClick={() => { handleShare(window.location.href, enqueueSnackbar); }}
                >
                  Share
                </Button>
              </Box>
