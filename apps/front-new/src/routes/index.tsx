...
const submissionSearchSchema = z.object({
   ...
   pageSize: z._default(z.coerce.number(), 10).check(z.minimum(10), z.maximum(50)),
});

const pageSizes = createListCollection({
   items: ['10', '20', '30', '40', '50'].map((value) => ({ label: value, value })),
});
...
export const Route = createFileRoute('/')({
   ...
   loader: ({ deps, context: { queryClient } }) =>
      queryClient.ensureQueryData(getSubmissionsOptions({ query: { ...deps } })),
   component: App,
});
                        ...
                        ? (
                           data?.data?.map((row, index) => (
                              <Table.Row key={index}>
                                 ...
                                 <Table.Cell>{row?.coop_year || 'N/A'}</Table.Cell>
                                 <Table.Cell>
                                    {row?.coop_cycle
                                       ? (
                                 ...
                                 <Table.Cell>{row?.program_level || 'N/A'}</Table.Cell>
                        ...
                        : (
                           <Table.Row>
                              <Table.Cell colSpan={8} textAlign='center' py={8}>
                                 <Text color='gray.500'>No salary data available</Text>
                              </Table.Cell>
                           </Table.Row>
                        )}
                  </Table.Body>
