...
import { course, course_corequisites, course_prerequisites } from '../tables';
   ...
   .as(qb =>
      qb
         .select({
            ...
            course_subject_id: sql<string>`${course.subject_id}`.as(
               'course_subject_id'
            ),
            course_number: sql<string>`${course.course_number}`.as(
               'course_number'
            ),
            prereq_id: sql<string>`${prereqCourse.id}`.as('prereq_id'),
            prereq_title: sql<string>`${prereqCourse.title}`.as('prereq_title'),
            prereq_subject_id: sql<string>`${prereqCourse.subject_id}`.as(
               'prereq_subject_id'
            ),
            prereq_course_number: sql<string>`${prereqCourse.course_number}`.as(
   ...
   .as(qb =>
      qb
         .select({
            ...
            coreq_subject_id: sql<string>`${coreqCourse.subject_id}`.as(
               'coreq_subject_id'
            ),
            coreq_course_number: sql<string>`${coreqCourse.course_number}`.as(
               'coreq_course_number'
            )
         })
