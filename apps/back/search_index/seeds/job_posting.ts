...
export default async (meilisearch: MeiliSearch, index: string) => {
   ...
   while (true) {
      ...
      const jobPostingsWithMajorsPromises = jobPostingsRaw.map(
         async (posting) => {
      ...
      console.log(
         `Indexed batch ${batchCount} (${jobPostings.length} job postings)`,
      );
   }

   console.log(
      `Completed indexing ${totalIndexed} job postings in ${batchCount} batches`,
   );
};
