    ...
    async ({
      ...
      try {
        ...
        if (result) {
          ...
          const normalizedInstructionType =
            normalizeInstructionType(instruction_type);
          const normalizedInstructionMethod =
            normalizeInstructionMethod(instruction_method);
...
export const section_function = async (drexel_section, term) => {
  ...
  try {
    ...
    for (let i = 0; i < sections_.length; i += batchSize) {
      ...
      console.log(
        `Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(sections_.length / batchSize)} for term ${term}`,
      );
