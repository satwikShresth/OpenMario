...
export function useConflicts(currentTerm: string, currentYear: number) {
   ...
   const allScheduledCourseIds = useMemo(() => {
      ...
      return [
         ...new Set(allCourses.map((e: any) => e.courseId).filter(Boolean))
      ];
   }, [allEvents, currentTerm, currentYear]);
   ...
   const scheduledCourseIds = useMemo(() => {
      return [
         ...new Set(scheduledEvents.map((e: any) => e.courseId).filter(Boolean))
      ];
   }, [scheduledEvents]);
   ...
   const conflicts = useMemo((): Conflict[] => {
      ...
      for (let i = 0; i < scheduledEvents.length; i++) {
         for (let j = i + 1; j < scheduledEvents.length; j++) {
            ...
            if (
               ...
               if (
                  !issues.find(
                     i => i.courseId === e1.courseId && i.type === 'overlap'
                  )
               ) {
                  issues.push({
      ...
      requisitesQueries.forEach((query, index) => {
         const event = scheduledEvents.find(
            (e: any) => e.courseId === scheduledCourseIds[index]
         );
         if (!event) return;
      ...
      const takenCourseIds = new Set(
         coursesTaken?.map((c: any) => c.courseId) || []
      );
      ...
      requisitesQueries.forEach((query, index) => {
         ...
         if (data?.prerequisites && data.prerequisites.length > 0) {
            ...
            data.prerequisites.forEach((group: any[]) => {
               ...
               const groupSatisfied = group.some((prereq: any) =>
