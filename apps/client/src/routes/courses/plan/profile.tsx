...
import { GpaPlanner } from '@/components/MajorMap';
import { BS_CS } from '@/lib/major-map';

export const Route = createFileRoute('/courses/plan/profile')({
  beforeLoad: () => ({ getLabel: () => 'My Courses' }),
...
function RouteComponent() {
  ...
  const isCourseDetail = useMatch({ from: '/courses/plan/profile/$course_id', shouldThrow: false });
  ...
  const handleAddCourse = async (course: any, status: 'taken' | 'considering') => {
    try {
      ...
      if (existing) {
        toaster.create({ title: 'Course Already Added', description: `${course.course} is already in My Courses`, type: 'warning' });
        return;
      }
  ...
  const handleRowClick = (courseId: string) => {
    navigate({ to: '/courses/plan/profile/$course_id', params: { course_id: courseId } });
  };
      ...
      <Courses.Root>
        <Flex direction='column' gap={1} mb={1}>
          ...
          <Text fontSize='sm' color='fg.muted'>
            Log courses, manage status, and plan GPA what-ifs in one place.
          </Text>
        </Flex>

        <Tabs.Root defaultValue='courses' variant='line' lazyMount>
          <Tabs.List mb={3}>
            <Tabs.Trigger value='courses'>Courses</Tabs.Trigger>
            <Tabs.Trigger value='gpa'>GPA planner</Tabs.Trigger>
          </Tabs.List>
          ...
          <Tabs.Content value='gpa'>
            <GpaPlanner program={BS_CS} />
          </Tabs.Content>
        </Tabs.Root>
      </Courses.Root>
    </>
