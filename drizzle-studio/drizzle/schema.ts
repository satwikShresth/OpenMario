import { pgTable, varchar, text, integer, time, foreignKey, serial, numeric, pgView } from "drizzle-orm/pg-core"
...
export const allCourseInstructorData = pgView("all_course_instructor_data", {
	...
	endTime: time("end_time"),
	days: text(),
}).as(sql`SELECT i.id AS instructor_id, i.name AS instructor_name, i.rmp_id AS instructor_rmp_id, i.avg_difficulty, i.avg_rating, i.num_ratings, c.crn AS course_id, c.subject_code, c.course_number, c.instruction_type, c.instruction_method, c.section, c.enroll, c.max_enroll, c.course_title, c.credits, c.prereqs, c.start_time, c.end_time, c.days FROM courses c LEFT JOIN course_instructor ci ON c.crn = ci.course_id LEFT JOIN instructors i ON i.id = ci.instructor_id`);
