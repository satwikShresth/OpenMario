...
import { Typography, Button, Box, Alert, Paper, Tabs, Tab } from '@mui/material';
...
function SubmissionsPageComponent() {
  ...
  const processFile = useCallback(async (file: File, common: CommonData) => {
    ...
    try {
      ...
      if (processedJobs.length > 0) {
        enqueueSnackbar(`${processedJobs.length} jobs added as drafts`, { variant: "success" });
        setTabValue(1);
      } else {
        enqueueSnackbar("No jobs detected in the uploaded file", { variant: "warning" });
      }
      ...
      {showDraftsAlert && tabValue !== 1 && (
        <Box sx={{ mb: 2 }}>
          <Alert severity="info" sx={{ mb: 2 }}>
            You have {draftSubmissions.length} draft submissions. Switch to the "Drafts" tab to view and complete them.
          </Alert>
        </Box>
      )}
