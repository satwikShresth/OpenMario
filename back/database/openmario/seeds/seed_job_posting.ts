      ...
      try {
         ...
         const jobPostingResult = await db.insert(job_posting)
            .values({
               ...
               minimum_gpa: job['Minimum GPA'] !== '- None -'
                  ? parseFloat(job['Minimum GPA'])
                  : null,
         ...
         console.log(`Job Posting: ${jobPostingId || 'N/A'}`);

         // 5. Add experience levels
         if (jobPostingId && job.Qualifications['Experience Levels']) {
            for (
         ...
         if (jobPostingId && job.Qualifications['Majors Sought']) {
         ...
         console.error(
            `Error processing job ${job.Position} at ${job.Employer}:`,
            error,
         );
      }
   }
