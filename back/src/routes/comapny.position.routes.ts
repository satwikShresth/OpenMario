...
import { CompanyPositionInsertSchema, CompanyPostionInsert, RequestParamsId } from '#models';
import { company, db, position } from '#db';
import { and, eq, or, sql } from 'drizzle-orm';
import { validateUser, zodBodyValidator } from '#/middleware/validation.middleware.ts';
            ...
            try {
               const company_id = await db
                  ...
                  .then(async ([existingCompany]) =>
                     existingCompany ? existingCompany.id : await db
                        .insert(company)
               ...
               return await db
                  ...
                  .then(async ([existingCompany]) =>
                     existingCompany ? res.status(409).json({ message: 'Position Already Exists' }) : await db
                        .insert(position)
                        .values({
                           name: position_name,
                           company_id,
