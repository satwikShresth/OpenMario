...
export default () => {
    ...
    */
   router
      .get('/', zValidator('query', SubmissionQuery), async (c) => {
       ...
       */
      .post('/', zValidator('json', SubmissionAggregateSchema), async (c) => {
       ...
       */
      .patch('/', zValidator('json', SubmissionAggregateSchema), async (c) => {
         const user_id = c.get('jwtPayload')?.user_id || null;
