...
async function scrapeCourses(subjectCode, httpClient, baseUrl) {
  const url =
    `${baseUrl}/coursedescriptions/quarter/undergrad/${subjectCode.toLowerCase()}/`;
  ...
  try {
    ...
    $(".courseblock").each((i, element) => {
      try {
        ...
        const titlePattern =
          /([A-Z]+)\s*([A-Z]?\d+[A-z]?)\s+(.*)\s+(\d+(?:\.\d+)?(?:-\d+(?:\.\d+)?)?)\s+Credits?/i;
        ...
        // College/Department
        const collegeMatch = metadataText.match(
          /College\/Department:\s*([^]*?)(?:Repeat Status:|Restrictions:|Prerequisites:|Corequisites?:|$)/,
        );
        if (collegeMatch) {
        ...
        // Repeat Status
        const repeatMatch = metadataText.match(
          /Repeat Status:\s*([^]*?)(?:Restrictions:|Prerequisites:|Corequisites?:|$)/,
        );
        if (repeatMatch) {
        ...
        // Restrictions
        const restrictionsMatch = metadataText.match(
          /Restrictions:\s*([^]*?)(?:Prerequisites:|Corequisites?:|$)/,
        );
        ...
        // Prerequisites
        const prerequisitesMatch = metadataText.match(
          /Prerequisites:\s*([^]*?)(?:Corequisites?:|$)/,
        );
        ...
        const corequisitesMatch = metadataText.match(
          /Corequisites?:?\s*([^]*?)$/,
        );
...
async function scrapeAllSubjects(flattenedSubjects, maxConcurrentRequests = 3) {
  ...
  console.log(
    `Processing ${subjectCodes.length} subjects in ${batches.length} batches`,
  );
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    const batchPromises = batches[i].map(async (subjectCode) => {
      console.log(
        `Starting to process ${subjectCode} - ${
          flattenedSubjects[subjectCode].name
        }`,
      );
      ...
      try {
        ...
        while (retries > 0) {
          try {
            ...
            console.log(
              `Retrying ${subjectCode} (${retries} attempts left)...`,
            );
            await delay(2000);
          }
        }
...
async function main() {
  try {
    ...
    console.log(
      `Prepared ${Object.keys(flattenedSubjects).length} subjects for scraping`,
    );
    ...
    const totalCourses = Object.values(allCourses).reduce(
      (sum, subject) => sum + subject.courses.length,
      0,
    );

    if (totalSubjects === 0 || totalCourses === 0) {
      console.warn(
        "Warning: No courses or subjects were scraped. Data may be incomplete.",
      );
    }
    ...
    if (emptySubjects.length > 0) {
      console.warn(
        `Warning: ${emptySubjects.length} subjects had zero courses:`,
        emptySubjects,
      );
    }
    ...
    if (
      typeof allCourses !== "undefined" && Object.keys(allCourses).length > 0
    ) {
